...
def get_cerebellum_labels(path):
    with open(path, 'r') as f:
...
def get_hippocampus_labels(path):
    with open(path, 'r') as f:
...
def get_resampled_image(img_path):
    # takes a label volume generated by freesurfer and transforms it in subject space
    
    orig = ants.image_read(T1_path)
    new = ants.image_read(img_path)
...
if __name__ == "__main__":
    ...
    T1 = nib.load(T1_path)
    fsLUT = get_freesurfer_LUT(os.path.join(output_dir, f'freesurfer/sub-{subject}/FreeSurferColorLUT.txt'))[1]
    ...
    for nparcels in [100, 200, 300, 400, 500, 600, 700, 800, 900, 1000]:
        ...
        #################################################### CEREBELLUM ATLAS
        cerebellum = nib.load(os.path.join(output_dir,f'cerebellum/sub-{subject}/nonlinear_gray_labels.nii.gz')).get_fdata().astype(int)

        old_labels = [(x,'cerebellum_'+get_cerebellum_labels(os.path.join(output_dir, f'cerebellum/sub-{subject}/LABELS.csv'))[x].replace(' ', '_')) for x in np.unique(cerebellum).tolist() if get_cerebellum_labels(os.path.join(output_dir, f'cerebellum/sub-{subject}/LABELS.csv'))[x]!='Unknown']
        ...
        GM_lab = np.load(os.path.join(output_dir, f'surfaces/sub-{subject}/cereb_inner_processed_original_labels.npy'))
        GM_lab = relabel_vector(GM_lab, dict(zip(*old_labels)), cereb_labels)
        np.save(os.path.join(output_dir, f'atlas/sub-{subject}/cerebellum_surf/cereb_labels.npy'), GM_lab)
        ...
        #################################################### ASEG ATLAS
        aseg = get_resampled_image(os.path.join(output_dir, f'freesurfer/sub-{subject}/mri/schaefer{nparcels}_aparc+aseg.mgz'))
        ...
        to_keep = [key for (key, val) in get_freesurfer_LUT(os.path.join(output_dir, f'freesurfer/sub-{subject}/FreeSurferColorLUT.txt'))[0].items() if val in labels_to_keep]
        ...
        schaefer_LUT = get_schaefer_LUT(os.path.join(output_dir, f'freesurfer/sub-{subject}/Schaefer_LUT/Schaefer2018_{nparcels}Parcels_17Networks_order_LUT.txt'))[1]
        ...
        _, _, values = nib.freesurfer.read_annot(os.path.join(output_dir, f'freesurfer/sub-{subject}/label/lh.Schaefer2018_{nparcels}Parcels_17Networks_order.annot'))
        ...
        _, _, values = nib.freesurfer.read_annot(os.path.join(output_dir, f'freesurfer/sub-{subject}/label/rh.Schaefer2018_{nparcels}Parcels_17Networks_order.annot'))
        values = np.array(values).astype(str)
        values[0] = 'Unknown'
        old_labels = dict(zip(range(len(values)+1), values.tolist()))
        fs_lab = relabel_vector(fs_lab, old_labels, newLabels)
        np.save(os.path.join(output_dir, f'atlas/sub-{subject}/freesurfer_surf/rh.{nparcels}Parcels_labels.npy'), fs_lab)
        ...
        ###################################################### BRAINSTEM ATLAS
        brstem = get_resampled_image(os.path.join(output_dir, f'freesurfer/sub-{subject}/mri/brainstemSsLabels.mgz'))
        ...
        ####################################################### THALAMUS ATLAS
        thalamus = get_resampled_image(os.path.join(output_dir, f'freesurfer/sub-{subject}/mri/ThalamicNuclei.mgz'))

        labels_to_keep = [val for (key,val) in get_freesurfer_LUT(os.path.join(output_dir, f'freesurfer/sub-{subject}/FreeSurferColorLUT.txt'))[0].items() if key in range(8103,8237)]
        to_keep = [key for (key, val) in get_freesurfer_LUT(os.path.join(output_dir, f'freesurfer/sub-{subject}/FreeSurferColorLUT.txt'))[0].items() if val in labels_to_keep]
        ...
        #################################################### AMYGDALA ATLAS
        ############## LEFT
        Lhippoamy = get_resampled_image(os.path.join(output_dir, f'freesurfer/sub-{subject}/mri/lh.hippoAmygLabels.mgz'))

        old_labels = [(x,'Left-'+get_freesurfer_LUT(os.path.join(output_dir, f'freesurfer/sub-{subject}/FreeSurferColorLUT.txt'))[0][x]) for x in range(7001,7021)]
        old_labels = list(zip(*old_labels))
        ...
        ############## RIGHT
        Rhippoamy = get_resampled_image(os.path.join(output_dir, f'freesurfer/sub-{subject}/mri/rh.hippoAmygLabels.mgz'))

        old_labels = [(x,'Right-'+get_freesurfer_LUT(os.path.join(output_dir, f'freesurfer/sub-{subject}/FreeSurferColorLUT.txt'))[0][x]) for x in range(7001,7021)]
        old_labels = list(zip(*old_labels))
        ...
        surf_lab = np.load(os.path.join(output_dir, f'surfaces/sub-{subject}/hippunfold_L_hipp_middle_original_labels.npy'))
        surf_lab = relabel_vector(surf_lab, dict(zip(*old_labels)), hipp_labels)
        np.save(os.path.join(output_dir, f'atlas/sub-{subject}/hippunfold_surf/L_hipp_labels.npy'), surf_lab)
        
        surf_lab = np.load(os.path.join(output_dir, f'surfaces/sub-{subject}/hippunfold_L_dentate_middle_original_labels.npy'))
        surf_lab = relabel_vector(surf_lab, dict(zip(*old_labels)), hipp_labels)
        np.save(os.path.join(output_dir, f'atlas/sub-{subject}/hippunfold_surf/L_dent_labels.npy'), surf_lab)
        ...
        surf_lab = np.load(os.path.join(output_dir, f'surfaces/sub-{subject}/hippunfold_R_hipp_middle_original_labels.npy'))
        surf_lab = relabel_vector(surf_lab, dict(zip(*old_labels)), hipp_labels)
        np.save(os.path.join(output_dir, f'atlas/sub-{subject}/hippunfold_surf/R_hipp_labels.npy'), surf_lab)
        
        surf_lab = np.load(os.path.join(output_dir, f'surfaces/sub-{subject}/hippunfold_R_dentate_middle_original_labels.npy'))
        surf_lab = relabel_vector(surf_lab, dict(zip(*old_labels)), hipp_labels)
        np.save(os.path.join(output_dir, f'atlas/sub-{subject}/hippunfold_surf/R_dent_labels.npy'), surf_lab)
        ...
        nib.save(atlas, os.path.join(output_dir,f'atlas/sub-{subject}/atlas{nparcels}.nii.gz'))
        
        ######################################################### SAVE LABELS
        with open(os.path.join(output_dir, f'atlas/sub-{subject}/atlas{nparcels}_labels.txt'), 'w') as f:
            for key,val in sorted(list(newLabels.items()), key=lambda x: x[0]):
                f.write(f'{key},{val}\n')
        
        ######################################################## SAVE LUT
        with open(os.path.join(output_dir, f'atlas/sub-{subject}/atlas{nparcels}_LUT.txt'), 'w') as f:
    ...
    # easy to read labels
    nparcels = 100
    schaefer_labels = get_schaefer_LUT(os.path.join(output_dir, f'freesurfer/sub-{subject}/Schaefer_LUT/Schaefer2018_{nparcels}Parcels_17Networks_order_LUT.txt'))[0]
    aggregated_labels = [   ([0,28,60,170,171,172,173,174,175,177,178,179,250,251,252,253,254,255,961,962,964,965,967,968,969,970,971,972,973,974,975, 1000, 2000],'Unknown'),
                            ...
                            ([7001,7002,7003,7004,7005,7006,7007,7008,7009,7010,7011,7012,7013,7014,7015,7016,7017,7018,7019,7020], 'Left-Amygdala'),
                            ([7021,7022,7023,7024,7025,7026,7027,7028,7029,7030,7031,7032,7033,7034,7035,7036,7037,7038,7039,7040], 'Right-Amygdala'),
                            ([8103,8104,8105,8106,8108,8109,8110,8111,8112,8113,8115,8116,8117,8118,8119,8120,8121,8122,8123,8125,8126,8127,8128,8129,8130,8133,8134,8135,8136], 'Left-Thalamus'),
                            ([8203,8204,8205,8206,8208,8209,8210,8211,8212,8213,8215,8216,8217,8218,8219,8220,8221,8222,8223,8225,8226,8227,8228,8229,8230,8233,8234,8235,8236], 'Right-Thalamus')]
    
    for lab, name in schaefer_labels:
        if lab>1000 and lab<2000:
            aggregated_labels.append(([lab], name))
        if lab>2000 and lab<3000:
            aggregated_labels.append(([lab], name))
    
    aggregated_labels = sorted(aggregated_labels, key = lambda x: x[0][0])
    
    atlas = nib.load(os.path.join(output_dir,f'atlas/sub-{subject}/atlas{nparcels}.nii.gz')).get_fdata().astype(int)
    with open(os.path.join(output_dir, f'atlas/sub-{subject}/atlas_aggregated_labels.txt'), 'w') as f:
        lab = 0
        for values, name in aggregated_labels:
            ...
            lab+=1
    atlas = nib.Nifti1Image(atlas, T1.affine, T1.header)
    nib.save(atlas, os.path.join(output_dir,f'atlas/sub-{subject}/atlas_aggregated.nii.gz'))

    import json
    with open(os.path.join(output_dir,f'atlas/sub-{subject}/atlas_to_aggregated.json'), 'w') as f:
